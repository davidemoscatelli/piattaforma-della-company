...
class Trek(models.Model):
    ...
    # Aggiunto per la API di FullCalendar, puoi rimuoverlo se non implementi il calendario a griglia
...
class ScheduledTrek(models.Model):
    ...
    is_active = models.BooleanField(
        default=True,
        verbose_name="Visibile Pubblicamente",
        help_text="Indica se l'uscita deve apparire nelle liste pubbliche delle prossime uscite. Impostare a No per uscite annullate o completate."
    )
    registration_open = models.BooleanField(default=True, verbose_name="Iscrizioni Aperte")
    
    event_status = models.CharField( # NUOVO CAMPO
        max_length=30,
        choices=EVENT_STATUS_CHOICES,
        default='PIANIFICATO',
        verbose_name="Stato Uscita"
    )
    ...
    def current_participants_count(self):
        # Conta solo le registrazioni CONFERMATE (le nuove iscrizioni sono ora CONFERMATE di default)
        return self.registrations.filter(status='CONFERMATA').count()

    @property
    def spots_available(self):
        # Assicurati che non vada sotto zero se per errore ci fossero più iscritti del max
        available = self.max_participants - self.current_participants_count
        return max(0, available)

    def save(self, *args, **kwargs):
        # Logica per aggiornare is_active e registration_open in base a event_status
        if self.event_status in ['ANNULLATO_METEO', 'ANNULLATO_ORGANIZZATORE', 'COMPLETATO']:
            self.is_active = False
            self.registration_open = False # Chiudi anche le iscrizioni se l'evento è annullato/completato
        elif self.event_status in ['PIANIFICATO', 'CONFERMATO']:
            # Se lo stato torna ad essere attivo, ripristina is_active e registration_open
            # ma solo se la data dell'evento non è passata (per CONFERMATO/PIANIFICATO)
            if self.date >= timezone.now().date():
                self.is_active = True
                # Non riaprire automaticamente le iscrizioni qui, potrebbe essere una decisione manuale
                # self.registration_open = True # Commentato: gestisci l'apertura iscrizioni manualmente
            else: # Se la data è passata, anche se confermato/pianificato, non dovrebbe essere attivo
                self.is_active = False
...
class TrekRegistration(models.Model):
    STATUS_CHOICES = [
        ('CONFERMATA', 'Confermata'), # Default ora
        ('ANNULLATA_UTENTE', 'Annullata (Utente)'),
        ('ANNULLATA_ADMIN', 'Annullata (Admin)'),
        # ('IN_ATTESA', 'In Attesa'), # Rimosso come opzione principale se non necessario
        ('COMPLETATA', 'Completata'), # Se si vuole tracciare chi ha partecipato effettivamente
    ]
    ...
    user = models.ForeignKey(User, on_delete=models.SET_NULL, blank=True, null=True, verbose_name="Utente Registrato", help_text="Collegato se l'utente era loggato durante l'iscrizione.")
    ...
    status = models.CharField(
        ...
        default='CONFERMATA',  # MODIFICA: Nuove iscrizioni sono subito CONFERMATE
    ...
    def __str__(self):
        if self.user:
            return f"Iscrizione di {self.user.username} a {self.scheduled_trek}"
        return f"Iscrizione di {self.first_name} {self.last_name} ({self.email}) per {self.scheduled_trek}"
